...
1. **Chain up.** Any service inactive, or no `/data/deployment-manifest.json`
   → `deploy-pi.sh` (resume, or a ~35 min first-run deploy).
2. **Chain matches the manifest.** `v1-l2/deployment.json` and the manifest
   must name the same FeeDistribution, it must exist on the L2 node, and the
   TokenPortal must have code on anvil. An L2 reset (see "Known limitation"
...
It prints per-step timings. Measured 2026-09-24 on a live, collateralised
